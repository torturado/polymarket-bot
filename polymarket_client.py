...
def robust_http_get(
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning(f"HTTP timeout (attempt {attempt + 1}/{max_retries}): {url}")
...
class PolymarketClient:
    ...
    def __init__(self, config: BotConfig):
        ...
        # WebSocket client for real-time BTC up/down markets
        self.ws_client = None
        try:
            import websockets
            from websocket_client import PolymarketWebSocketClient
            self.ws_client = PolymarketWebSocketClient(config)
            logger.info("WebSocket client initialized for real-time BTC up/down markets")
        except ImportError:
            logger.warning("WebSocket client not available. Install websockets: pip install websockets")
        except Exception as e:
    ...
    def list_btc_markets(self) -> list[Market]:
        ...
        return self._list_btc_markets_from_api()

    def _list_btc_markets_from_api(self) -> list[Market]:
        """
        Fetch BTC 15-minute up/down markets using Gamma Events API.
        ...
        try:
            ...
            try:
                for offset in [0, 100, 200]:
                    ...
                    logger.debug(f"Fetched {len(events)} events (offset {offset}), total markets: {len(all_markets)}")
            ...
            # Strategy 2: Fetch BTC up/down markets directly by searching
            # This catches markets that might be "closed" for new positions but still active
            try:
                # Search specifically for BTC up/down 15m markets
                search_params = {
                    "limit": 100,
                    "_q": "btc-updown-15m"  # Search query
                }

                response = robust_http_get(markets_url, params=search_params)
                if not response:
                ...
                logger.debug(f"Added {len(direct_markets)} from markets search endpoint")

            except Exception as e:
                logger.warning(f"Error fetching direct markets: {e}")

            # Strategy 3: Calculate and fetch the CURRENT market directly by slug
            try:
                now_ts = int(datetime.now(timezone.utc).timestamp())
                current_window_start = (now_ts // 900) * 900  # Round down to 15-min boundary
                current_slug = f"btc-updown-15m-{current_window_start}"

                # Try to fetch the current market directly
                response = robust_http_get(f"{markets_url}?slug={current_slug}")
                if response and response.status_code == 200:
                    current_markets = response.json()
                    if current_markets:
                        for market in (current_markets if isinstance(current_markets, list) else [current_markets]):
                            cid = market.get("conditionId") or market.get("condition_id")
                            if cid and cid not in seen_condition_ids:
                                all_markets.append(market)
                                seen_condition_ids.add(cid)
                                logger.info(f"✅ Found current market: {current_slug}")

                # Also try next market
                next_window_start = current_window_start + 900
                next_slug = f"btc-updown-15m-{next_window_start}"
                response = robust_http_get(f"{markets_url}?slug={next_slug}")
                if response and response.status_code == 200:
                    next_markets = response.json()
                    if next_markets:
                        for market in (next_markets if isinstance(next_markets, list) else [next_markets]):
                            cid = market.get("conditionId") or market.get("condition_id")
                            if cid and cid not in seen_condition_ids:
                                all_markets.append(market)
                                seen_condition_ids.add(cid)
                ...
                logger.debug(f"Error fetching current market by slug: {e}")
            ...
            logger.debug(f"Received {len(markets_data)} total markets from Gamma API")
            ...
            for market_data in markets_data:
                try:
                    ...
                    slug = market_data.get("slug", "").lower() if market_data.get("slug") else ""
                    title = market_data.get("title", "").lower() if market_data.get("title") else ""

                    # Check for BTC up/down 15-minute markets SPECIFICALLY
                    # These have slugs like "btc-updown-15m-1765358100"
                    slug = market_data.get("slug", "").lower()

                    # STRICT filter: Only match the specific 15-minute up/down markets
                    is_btc_updown_15m = (
                        "btc-updown-15m" in slug or
                        "bitcoin-updown-15m" in slug or
                        ("btc" in slug and "updown" in slug and "15m" in slug)
                    )

                    # Log for debugging
                    if is_btc_updown_15m and len(markets) < 5:
                        logger.debug(
                            f"✅ Found BTC 15m market - Slug: {slug} | "
                            f"Question: {market_data.get('question', '')[:50]}..."
                        )

                    # ONLY accept 15-minute up/down markets
                    if not is_btc_updown_15m:
                        continue
                    ...
                    if not token_ids and outcomes:
                        if isinstance(outcomes[0], dict):
                            token_ids = [o.get("token_id") or o.get("tokenId", "") for o in outcomes if o.get("token_id") or o.get("tokenId")]
                    ...
                    if outcomes:
                        if isinstance(outcomes[0], dict):
                            ...
                            for idx, outcome in enumerate(outcomes):
                                outcome_name_lower = (outcome.get("name", "") or outcome.get("title", "")).lower()
                                if "up" in outcome_name_lower and "down" not in outcome_name_lower:
                                    side = MarketSide.UP
                                    outcome_name = outcome.get("name") or outcome.get("title")
                    ...
                    # Format: btc-updown-15m-1765359000 -> timestamp is START time, add 900 for end
                    if not end_date and slug:
                        import re
                        slug_match = re.search(r'btc-updown-15m-(\d+)', slug)
                        if slug_match:
                            try:
                                start_timestamp = int(slug_match.group(1))
                    ...
                    if not end_date:
                        import re
                        question_text = market_data.get("question", "") or market_data.get("title", "")
                        ...
                        if time_match:
                            try:
                                ...
                                end_date_et = datetime(year, month, day, end_hour, end_minute, tzinfo=et_tz)
                    ...
                    if not token_id and token_ids:
                        if len(token_ids) >= 2:
                            token_id = token_ids[0] if side == MarketSide.UP else token_ids[1]
                    ...
                    market_question = market_data.get("question") or market_data.get("title", "")
                    ...
                    if outcomes and len(outcomes) >= 2 and condition_id:
                        ...
                        if isinstance(outcomes[0], dict):
                            for outcome in outcomes:
                                outcome_name_lower = (outcome.get("name", "") or outcome.get("title", "")).lower()
                                outcome_token_id = outcome.get("token_id") or outcome.get("tokenId")
                                ...
                                if opposite_side == MarketSide.UP and "up" in outcome_name_lower and "down" not in outcome_name_lower:
                                    ...
                                    opposite_token_id = outcome_token_id
                                    break
                        elif isinstance(outcomes[0], str) and len(token_ids) >= 2:
                        ...
                        if not opposite_token_id and len(token_ids) >= 2:
                            opposite_token_id = token_ids[1] if side == MarketSide.UP else token_ids[0]
                        ...
                        if opposite_token_id:
                            ...
                            if not any(
                                m.condition_id == condition_id and m.side == opposite_side
                                for m in markets
                            ):
                    ...
                    logger.warning(f"Failed to parse market: {e}", exc_info=True)
                    continue

            logger.info(f"Successfully fetched {len(markets)} BTC markets")
            return markets
        except Exception as e:
            logger.error(f"Failed to fetch markets: {e}", exc_info=True)
            return []

    def get_market_quote(self, market_id: str, side: MarketSide) -> Optional[Quote]:
        """
        ...
        try:
            ...
            try:
                price_data = self.clob_client.get_price(token_id, side=side_str)
    ...
    def find_market_pairs(self, only_current: bool = True) -> list[MarketPair]:
        ...
        for market in markets:
            ...
            if condition_id not in market_groups:
                market_groups[condition_id] = {"up": None, "down": None, "end_date": market.end_date}
        ...
        # Calculate what the current market SHOULD be based on timestamp
        now_ts = int(now.timestamp())
        current_window_start = (now_ts // 900) * 900  # Round down to 15-min boundary
        current_window_end = current_window_start + 900
        expected_slug = f"btc-updown-15m-{current_window_start}"
        expected_end = datetime.fromtimestamp(current_window_end, tz=timezone.utc)

        logger.info(f"📍 Current time: {now.strftime('%H:%M:%S UTC')} | Expected market: {expected_slug} (ends {expected_end.strftime('%H:%M:%S UTC')})")
        ...
        pairs = valid_pairs
        pairs.sort(key=lambda p: p._end_date if hasattr(p, '_end_date') and p._end_date else datetime.max.replace(tzinfo=timezone.utc))
        ...
        if pairs and logger.isEnabledFor(logging.DEBUG):
            for i, p in enumerate(pairs[:3]):
                ...
                ed_str = ed.strftime("%Y-%m-%d %H:%M UTC") if ed else "No end date"
                logger.debug(f"  #{i+1}: {p.up_market.question[:50]}... | End: {ed_str}")

        # If only_current, filter to just the market ending soonest
        if only_current and pairs:
            current_pair = pairs[0]
            end_time = current_pair._end_date if hasattr(current_pair, '_end_date') else None
            ...
            if end_time:
                ...
                logger.info(
                    f"🎯 TRACKING: {current_pair.up_market.question[:60]}... | "
                    f"Ends in {minutes_left}m {seconds_left}s | "
                    f"Condition: {current_pair.pair_id[:16]}..."
                )
            else:
                logger.info(f"🎯 TRACKING: {current_pair.up_market.question[:60]}... (no end date)")

            return [current_pair]

        return pairs

    def get_pair_quotes(self, pair: MarketPair) -> Optional[tuple[Quote, Quote]]:
        """Get quotes for both sides of a market pair using CLOB API."""
        up_quote = self.get_market_quote(pair.up_market.market_id, MarketSide.UP)
        down_quote = self.get_market_quote(pair.down_market.market_id, MarketSide.DOWN)
