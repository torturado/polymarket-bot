...
class PolymarketRTDSClient:
    ...
    def __init__(self, config: BotConfig):
        ...
        self.websocket = None
        self.running = False
        self.markets_cache: Dict[str, Dict[str, Any]] = {}  # asset_id -> market data
        ...
        self.has_credentials = all([self.api_key, self.api_secret, self.api_passphrase])

        if self.has_credentials:
            logger.info("🔑 RTDS: API credentials configured for authenticated streams")
        else:
            logger.info("📖 RTDS: Running in read-only mode (no API credentials)")
    ...
    async def subscribe_to_markets(self, asset_ids: List[str] = None) -> bool:
        ...
        if not asset_ids:
            asset_ids = await self._fetch_btc_market_tokens()
            ...
            "subscriptions": [
                {
                    ...
                    "type": "*",  # All message types (price_changes, agg_orderbook, last_trade_price)
                    "filters": filters
                }
            ]
        }
        ...
        try:
            ...
            logger.info(f"📡 RTDS: Subscribed to {len(asset_ids)} market tokens")
            return True
        except Exception as e:
    ...
    async def _handle_orderbook(self, payload: Dict[str, Any]):
        ...
        if bids:
            ...
            self.markets_cache[asset_id]["best_bid"] = float(sorted_bids[0].get("price", 0))
        if asks:
            sorted_asks = sorted(asks, key=lambda x: float(x.get("price", 999)))
            self.markets_cache[asset_id]["best_ask"] = float(sorted_asks[0].get("price", 0))
    ...
    async def _fetch_btc_market_tokens(self) -> List[str]:
        """Fetch BTC up/down market token IDs from Gamma API."""
        import aiohttp
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                ...
                async with session.get(url, params=params) as response:
                    if response.status != 200:
                        logger.warning(f"Failed to fetch events: {response.status}")
                        return []
                    ...
                    for event in events:
                        ...
                        if ("btc" in title or "bitcoin" in title) and ("up" in title or "down" in title):
                    ...
                    logger.info(f"Found {len(token_ids)} BTC market tokens")
                    return token_ids[:100]

        except Exception as e:
            logger.error(f"Error fetching BTC market tokens: {e}")
            return []
    ...
    if await client.connect():
        ...
        try:
            ...
            for asset_id, data in client.get_cached_markets().items():
                print(f"  {asset_id[:20]}... bid={data.get('best_bid')} ask={data.get('best_ask')}")
