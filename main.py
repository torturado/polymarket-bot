...
logging.getLogger("websocket_client").setLevel(logging.WARNING)  # Only show warnings/errors from WS
logging.getLogger("polymarket_client").setLevel(logging.INFO)  # Normal logging


def run_paper_bot():
    """Run the paper-trading bot."""
    print("Starting Polymarket BTC Paper-Trading Bot...")
    print()
    ...
    if has_creds:
        ...
        print(f"  Set POLYMARKET_API_KEY, POLYMARKET_API_SECRET, POLYMARKET_PASSPHRASE")
    print()

    print(f"Configuration (Leg-In Strategy):")
    print(f"  First leg threshold: ${config.first_leg_threshold:.2f} (buy when UP or DOWN ≤ this)")
    print(f"  Complete threshold: ${config.complete_threshold:.2f} (complete when total ≤ this)")
...
def main():
    """CLI entrypoint."""
    parser = argparse.ArgumentParser(description="Polymarket BTC Paper-Trading Bot")
    parser.add_argument(
        "command",
